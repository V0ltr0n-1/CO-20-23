...
/** repeating variable table 'TESTS' including variables to flag as chco***/
proc sql;
create table tests
...
proc sort data=tests; by LabSpecimenID; run;
...
/*skip to data Transforming step*/




/*May 2022 testing different joins and sorts to reduce # of repeating tests*/
...
if first.LabSpecimenID;
run;
proc sort data=all21; by LastName; run;
...
proc sort data=tests; by EventID; run;
...
/*Inner join for TESTS table*/

	proc sql;
create table tests_inner
as select distinct d.*, 
s.TestingLabName, s.OriginatingLabName, s.CollectionDate,s.Specimen, s.LabSpecimenID,
l.TestType, l.ResultDate, l.ResultText
	...
	inner join NewCEDRS.zDSI_Specimens s on d.EventID = s.EventID
	inner join NewCEDRS.zDSI_LabTests l on d.EventID = l.EventID 
...
proc export data=all21_dedupmj
               outfile='C:\Users\iaoyegun\Documents\CCC non-matches\CEDRS post jz\all21_dedupmj.CSV'	/*pause 2/8*********/	
...
data ch_dataTransftest /*(drop=Count)*/;
	set tests;
...
modify  ch_dataTransftest;
attrib _all_ label=' '; 
 
rename  FirstName=fname LastName=lname new_bdate=dob new_dtpcr=date_pcr pcr_res2=covid_pcr_result testlab=pcr_site;
 
		
contents data=ch_dataTransftest;
run;
	quit; /*skip to 2x export of ch_dataTransftest*/
...
proc export data=ch_dataTransftest
               outfile='C:\Users\iaoyegun\Documents\CCC non-matches\CEDRS post jz\ch_dataTransftest.XLS'		
      dbms=xls replace;    run;
...
data archive.retain_all21_mjtest;
	
	retain ProfileID EventID redcap_repeat_instrument redcap_repeat_instance fname lname dob date_pcr 
			covid_pcr_result pcr_site;
		

		set ch_dataTransftest;
	
	
	run; /*run next steps below*/
...
data archive.redcap_all21_mjtest;
	set archive.retain_all21_mjtest;

	Keep ProfileID EventID redcap_repeat_instrument redcap_repeat_instance fname lname dob date_pcr covid_pcr_result pcr_site;
...
proc export data=archive.redcap_all21_mjtest
               outfile='C:\Users\iaoyegun\Documents\CCC non-matches\CEDRS post jz\redcap_all21_mjtest.csv'		
      dbms=xls replace;    run;


	data archive.format_all21_mjtest2;
	set archive.redcap_all21_mjtest;

		Keep ProfileID EventID redcap_repeat_instrument redcap_repeat_instance fname lname dob date_pcr covid_pcr_result pcr_site;


	  format  covid_pcr_result $covid_pcr_result. pcr_site $pcr_site. ;

		run;


		ods excel file='C:\Users\iaoyegun\Documents\CCC non-matches\CEDRS post jz\format_all21_mjtest2.xls'	; /*older versions of SAS may replace this line with: ods tagsets.excelxp file=""*/
			proc report data=archive.format_all21_mjtest2;
				columns _all_;
			run;
		ods excel close;
		...
		/*end Tests repeating variables table*/
...
rename  FirstName=fname LastName=lname new_bdate=dob HospitalizedYesNO=admit AdmissionDate=admit_date died=death hospitalID=admit_hospital
		intermed_icu=icu DeathDate=date_death DischargeDate=dis_date ;
...
data archive.retain_all21_mjhosp;
	...
	retain EventID redcap_repeat_instrument fname lname dob admit admit_date admit_hospital icu death 
		date_death dis_date;
...
data archive.redcap_all21_mjhosp;
	...
	Keep EventID redcap_repeat_instrument fname lname dob admit admit_date admit_hospital icu death 
		date_death dis_date;
...
proc export data=archive.redcap_all21_mjhosp
	  ...
	  /*formatting*/
data archive.format_all21_mjhosp2;
	...
	Keep EventID redcap_repeat_instrument fname lname dob admit admit_date admit_hospital icu death 
	  ...
	  format  admit $text_ynu. icu $icu. death $death.;

		run;


ods excel file='C:\Users\iaoyegun\Documents\CCC non-matches\CEDRS post jz\format_all21_mjhosp2.xls'	; /*older versions of SAS may replace this line with: ods tagsets.excelxp file=""*/
			proc report data=archive.format_all21_mjhosp2;
				columns _all_;
			run;
		ods excel close;
