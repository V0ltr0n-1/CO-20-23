...
libname agencies odbc dsn='CEDRS_III_Warehouse' schema=access READ_LOCK_TYPE=NOLOCK;
...
proc contents data=ch21_msmj_apr2;
	 run;
proc export data=Agencies.Agencies 
 outfile='J:\Programs\Other Pathogens or Responses\2019-nCoV\Pediatric COVID-19 Registry\CEDRS Data\CCC 3_22\Agencies.csv'		
...
"White"=5 "Unknown or not reported"=7 " "=7 "Other"=8;*/
	value immunocomp 121=1 119=0 120=0 .=0;
	value $umc_yn "Yes"="1" "No"="2" "Unknown or undocumented"="3" " "="3";
	value YESNO  /*"No"=0 " "=0 "Yes"=1*/ 121=1 119=2 120=3 .=3;
...
s.TestingLabName, s.OriginatingLabName, s.LabSpecimenID, datepart(s.CollectionDate)as CollectionDate format mmddyy10., s.LabSpecimenID
/*l.TestType, l.ResultDate, l.ResultText*.*/
	...
	where d.disease ='COVID-19' and d.EventStatus in ('Confirmed')*/
	
	group by d.EventID;
	quit; 
	

proc sort data=tests1 nodupkey out=TestTypededup;
by LabSpecimenID;
run;
proc sort data=TestTypededup out=Test_Sort;
by EventID;
run;
/*data test_final;
	set Test_Sort;
	redcap_repeat_instrument=TestType;
	by EventID;
	retain order 0;
if first.EventID then order=order+1;
	run;*/
...
if '01MAR2020:00:00:00'dt <= ReportedDate <= '30JUN2020:00:00:00'dt; 
 ...
 proc sql;
	...
	group by s.EventID;
	quit;
proc sort data=tests2 nodupkey out=TestTypededup;
by LabSpecimenID;
run;
proc sort data=TestTypededup out=Test_Sort;
by EventID;
...
set childrens;
by EventID;
if '01MAR2020:00:00:00'dt <= ReportedDate <= '31JUL2020:00:00:00'dt; 
run;
 /*e.HospitalizedYesNo, e.AdmissionDate, e.DischargeDate, d.DeathDate, s.CollectionDate 
l.TestType l.ResultText s.TestingLabName,*/
/*proc sql;
 create table archive.admit
As select distinct
ProfileID, EventID, HospitalizedYesNo, ReportedDate
...
proc sql;
create table archive.testtype
as select distinct ProfileID, EventID, TestType, LabSpecimenID, ReportedDate 
from test_sort
group by LabSpecimenID;
quit;
data repeatstacks;
set archive.admit
	archive.testType;
...
redcap_repeat_instrument = CATS(HospitalizedYesNo, TestType);
/*Drop TestType HospitalizedYesNoif '01MAR2020:00:00:00'dt <= ReportedDate <= '30JUN2020:00:00:00'dt; 
 run;
...
format DiabetesID	ChronicRenalDiseaseID	ChronicLiverDiseaseID	ChronicOtherID UnderlyingMedicalConditionID PsychologicalID DisabilityID 
		...
		immunocomp. int trvlint. dom trvldom. OnsetDate mmddyy10.;


run;
Data ch21_2;
set ch21;
	Drop  ProfileID HospitalizedYesNo AdmissionDate icu died DeathDate HospitalID  
DischargeDate TestType CollectionDate ResultText TestingLabName;
run;
proc sort data=ch21_2 nodupkey out=Ch21_3;
by EventID; run;
                                                                   /////////*ADMIT TABLE*/////////;
proc sql;
 create table archive.admit_sheet
As select distinct
 EventID, HospitalizedYesNo, AdmissionDate, icu, died, DeathDate, DischargeDate, ReportedDate, newrace, LastName

 From ch21 
Group by EventID;
quit;
data admit_limits;
set archive.admit_sheet;
length redcap_repeat_instrument $100.;
redcap_repeat_instrument = HospitalizedYesNo;
if '01MAR2020:00:00:00'dt <= ReportedDate <= '31JUL2020:00:00:00'dt;
 
run;
data admit_instance;
set admit_limits;
length redcap_repeat_instance $100.;
by EventID;
if first.EventID then Count=.;
Count+1;
run;
data admit_instance_2;
set admit_instance;
redcap_repeat_instance=Count; 
if redcap_repeat_instrument='No' then delete;
if redcap_repeat_instrument=' ' then delete;
if redcap_repeat_instrument='Unknown' then delete;
if LastName=" " then delete;
by EventID;
format redcap_repeat_instrument $redcap_repeat_instrument.;
run;
proc datasets library=work;
attrib _all_ label=' ';
modify admit_instance_2;
 rename HospitalizedYesNO=admit AdmissionDate=admit_date icu=icu /*icudate*/ died=death hospitalID=admit_hospital
		DeathDate=date_death DischargeDate=dis_date newrace=race;  
contents data=admit_instance_2;
run;
	quit; 
	data admit_almost;
	set admit_instance_2;
	keep EventID  redcap_repeat_instrument  redcap_repeat_instance race admit admit_date icu death date_death dis_date;
	format icu $icu. death $death.  admit $admit. race newrace $empty.;
	run;
	proc sort data=admit_almost nodupkey out=admit_final;
	by EventID;
	run;
	/*proc sql ;
	CREATE TABLE admit.final_2
	AS SELECT DISTINCT PP.*, pr.race
	from admit_final PP
	left join ch21_msmj as pr on pp.EventID=pr.EventID
	order by EventID;
	quit;*/
...
proc sql;
create table archive.testtype_sheet
as select distinct  tt.EventID, tt.TestType, tt.CollectionDate, tt.ResultText, ti.TestLab, tt.LabSpecimenID, tt.ReportedDate, ti.testlab, ti.newrace, ti.LastName
from tests2 tt
left join ch21 as ti on tt.EventID=ti.EventID
order by EventID;
quit;
data testtype_limits;
set archive.testtype_sheet;
length redcap_repeat_instrument $100.;
redcap_repeat_instrument = TestType;
if '01MAR2020:00:00:00'dt <= ReportedDate <= '31JUL2020:00:00:00'dt; 
run;
data testtype_instance;
	set testtype_limits;
	length redcap_repeat_instance $100.;
	by EventID;
	if first.EventID then Count=.;
...
data TestType_instance_2;
	set testtype_instance;
	redcap_repeat_instance=Count;
	if redcap_repeat_instrument='Variant of public health concern' then delete;
	if redcap_repeat_instrument='Total Antibody' then delete;
	if redcap_repeat_instrument='Other Molecular Assay' then delete;
	if redcap_repeat_instrument='COVID-19 Variant Type' then delete;
	if LastName=" " then delete;
	if ResultText="Unknown" then delete;

	format redcap_repeat_instrument $redcap_repeat_instrument.  race ID_sx umc___2 immunocomp 
		umc___1 umc___3 umc___4 umc___5 umc___8 umc___9 endo_dx___1 cards_dx___4 weight_dx___1 personal_smoking 
		umc_yn school_daycare sx_asx  /* date_pcr MMDDYY10.  pcr_site $pcr_site. covid_pcr_result $covid_pcr_result.*/
		...
		sx_onset___18	sx_onset___19	sx_onset___20	sx_onset___21	sx_onset___22	sx_onset___25 sx_onset___26 covid_rf___1 
			covid_rf___2 covid_rf___9 $empty. ;
run;
proc datasets library=work;

	modify  TestType_instance_2;
 	rename  CollectionDate=date_pcr  ResultText=covid_pcr_result Testlab=pcr_site newrace=race;  
	contents data= TestType_instance_2;
run;
	quit; 
proc sort data=TestType_instance_2 nodupkey out=TestType_final;
by LabspecimenID;
run;
data Testtype_final;
	set TestType_instance_2;
	length pcr_site $100;
	keep    EventID redcap_repeat_instrument  redcap_repeat_instance date_pcr covid_pcr_result pcr_site race ;
	by eventID;
	format  date_pcr MMDDYY10.  pcr_site $pcr_site. covid_pcr_result $covid_pcr_result. race $empty.;

run;

/*proc sql;
create table Final_Redcap
as select distinct q.*, q.redcap_repeat_instrument, k.HospitalizedYesNo, k.AdmissionDate, k.icu, k.died, k.DeathDate, 
k.DischargeDate, g.TestType, g.CollectionDate, g.ResultText, g.TestingLabName, g.LabSpecimenID

from archive.RRI q
	left join archive.admit_sheet k on q.ProfileID = k.ProfileID
	left join archive.testtype_sheet g on q.ProfileID = g.ProfileID
	order by ProfileID;
	quit;
	proc sort data=Final_Redcap nodupkey out=Final_Redcap_Repeat;
	by _ALL_;
run;*/

/*proc sql;
create table ch21_Redcap_repeat
as select distinct y.*, x.redcap_repeat_instrument, x.HospitalizedYesNo, x.AdmissionDate, x.icu, x.died, x.DeathDate, 
x.DischargeDate, x.TestType, x.CollectionDate, x.ResultText, x.TestingLabName

from ch21_msmj y
left join Final_Redcap_Repeat x on y.ProfileID=x.ProfileID
order by ProfileID;
quit;
	data try1;
	set ch21_msmj
	Final_Redcap_Repeat;
	run;
	proc sort data=try1 nodupkey out=try2;
	by _ALL_;
run;*/
...
proc export data=ALLTHINGS
               outfile='J:\Programs\Other Pathogens or Responses\2019-nCoV\Pediatric COVID-19 Registry\CEDRS Data\CCC 3_22\ALLTHINGS.CSV'		
      dbms=csv replace;    run;
/*create data set of childrens cases not alredy in CHOC dataset*/******remove 19 CHCO identified from this delete list;
 data ch21_miss;
	  ...
	  run;
proc sort data=ch21_miss  nodupkey out=ch21_miss_2;
by EventID;
quit;
*subset missing to Mar-June cases;
data ch21_msmj ;
	drop state LabSpecimenID;
	set ch21_miss_2;
	record_id="NEW";
	by EventID;
...
length redcap_repeat_instance $100.;
retain order 0;
if first.redcap_repeat_instrument then order=order+1;
run;
*********proc sort data=ch21_msmj_apr2;
...
rename  cdrs_source=data_source___2 MedicalRecordNumber=mrn FirstName=fname LastName=lname BirthDate=dob Age=age AgeType=age_unit Gender=sex 
		...
		LungDiseaseID=umc___1 ChronicLiverDiseaseID=umc___2 DiabetesID=endo_dx___1 cardio=umc___4 
		ChronicRenalDiseaseID=umc___5 
        ...
        Symptoms=sx_asx OnsetDate=sx_onset_date FeverOver100_4=sx_onset___1 FeverChills=sx_onset___2 AnyCough=sx_onset___3 
...
data archive.reord_ch_mj;
	retain EventID record_id redcap_repeat_instrument redcap_repeat_instance EventID data_source___2 mrn fname lname dob age age_unit sex ethnic race
		...
		endo_dx___1 cards_dx___4 /*psych_dx psych_other gyn_dx weight_dx___1*/ personal_smoking covid_rf___1 covid_rf___2 covid_rf___9 /*int_travel dom_travel*/
	...
	set ch21_msmj;
		...
		IF card_dx___4=1 Then umc___4=1; if cardio=1 then umc___4=1;
	...
	format data_source__2 $DATA_SOURCE. age_unit $age_unit. ethnic $ethnic. sex $sex. /*race $race.*/  ID_sx umc___2 immunocomp 
		umc___1 umc___3 umc___4 umc___5 umc___8 umc___9 endo_dx___1 cards_dx___4 weight_dx___1 personal_smoking 
		...
		sx_onset_date mmddyy10. redcap_repeat_instrument $redcap_repeat_instrument.;

	run; 

	
888888remove the format step for umc_yn and sx yes no so i can add conditions
	/*data repeatable;
...
run;
	...
	proc export data= final
               outfile='J:\Programs\Other Pathogens or Responses\2019-nCoV\Pediatric COVID-19 Registry\CEDRS Data\CCC 3_22\final.CSV'		
      dbms=csv replace;    run;
